...
async function start() {
  try {
    ...
    server.listen(port, () => {
      ...
      console.log(Logger(`Ready in ${formattedTime}`, 'success', 'green')) // Compiled in
    });

  } catch (error) {
    console.log(Logger(`Failed to start server: ${error}`, 'error', 'red'));
    process.exit(1);
  }
};
...
async function shutdown() {
  ...
  try {
    ...
    console.log(Logger(`Error connecting database`, 'warning', 'yellow'))
